...
24. **M5a GitHub 同步（进行中）**：引擎完成（VelaGitHub.syncNow：自动建私有仓库 vela-sync → 收藏树序列化 → URL 并集合并 → Contents API SHA 乐观锁推送；PlacesUtils 观察者 5s 防抖；启动自动同步）。设置页卡片已注册（account-sync.mjs githubSync 组 + main.js addSetting 处理器 + zh-CN/en-US 文案入包）——**渲染门槛已破（见坑 28：fluent 值形式清空子元素）**。**过渡可用**：浏览器控制台执行 `VelaGitHub.beginLogin()`（输授权码登录）→ `VelaGitHub.syncNow()`（自动建仓+同步）。
...
28. **设置页卡片"只剩标题不渲染条目"真凶 = fluent ftl 条目写成了值形式**：`github-sync-group = GitHub 同步`（值）会让 Fluent 在应用翻译时把值写入 textContent 并**按规范清掉该元素全部子元素**——setting-control 们就这么"被删"了（textContent 原生替换，绕过一切 JS 钩子；MutationObserver 只能看到尸体）。正确写法是**属性形式**：`github-sync-group =\n    .label = GitHub 同步`，条目控件对照：placeholder-message 用 `.label`+`.description`，moz-box-button 用 `.label`，moz-box-item 用 `.label`+`.description`。三处都改：overlay/l10n zh-CN preferences.ftl（全文收编）+ vela-src en-US preferences.ftl（apply.sh 幂等改写步骤固化）。**双层缓存坑**：①改 l10n-central 后必须 `./mach build langpack-zh-CN` 重打语言包（普通 mach build 不重打）；②profile 首启会把 distro 语言包**拷贝进自己 extensions/**，此后永远用旧拷贝——install-langpack.sh 现已每次删除 profile 侧拷贝。诊断链路：探针证明控件 visible=true 却 rect=0x0 → 对照正常组 → 读 ftl 写法才破案。
29. **GitHub 登录链路四连 bug（"点登录没反应"全解）**：①`window.addEventListener(() => ...)` 把函数当事件名传，init 永不执行——第一参数必须是事件名字符串（"load"）且要处理 readyState；②esr153 **删除了 `Services.logins.findLogins()`**（NS_ERROR_NOT_IMPLEMENTED），必须用 `searchLoginsAsync({ origin })`——令牌存/取/删全换；③preferences 页签是**独立 chrome window**，ES 模块里裸 `VelaGitHub` 不可达、`window.VelaGitHub` 也不是主窗口——统一 `Services.wm.getMostRecentWindow("navigator:browser")?.VelaGitHub`；④菜单项内联 oncommand 改为模块加载时编程式 `addEventListener("command")` 绑定（vela-menu-login/sync/logout）。实测闭环：设置页点登录 → 设备流 fetch → 授权码弹窗 + github.com/login/device 页自动打开（日志 VELA_GH device code ready）。
30. **磁贴双开 + 顶栏菜单点击无效 = frame script 双注入 + 绑定早于 DOM**：①vela-gestures.js 与 vela-superdrag.js **各自 loadFrameScript 同一 vela-framescript.js**（loadFrameScript 不去重）→ 点击/手势监听器翻倍——磁贴一次点击发两条 Vela:SuperDrag=开两个标签、手势消息翻倍=动作执行两次。修=messageManager 上挂 `__velaFramescriptLoaded` 旗标只注入一次。②global-scripts.js 在 browser.xhtml **第 103 行**执行而 toolbox 标记（含 vela-menu-* 菜单项）**第 137 行**才出现——bindToolbarMenu 的 getElementById 全 null 静默不绑（顶栏菜单死因，加载日志无任何报错）。修=绑定延到 load 事件；加 `VELA_GH menu bound N/3` 绑定计数日志（实测 0/3→3/3）。**凡是 global-scripts 里 loadSubScript 的模块，凡引用后置 DOM 的 getElementById 必须延到 load**。设备流 fetch 加 AbortSignal.timeout(15000)（大陆网络挂起时明确报错替代无声等待）。
31. **鼠标手势出厂关闭（用户反馈"左键画线很烦"）**：branding pref `vela.gestures.enabled=false`（代码保留，about:config 可开回）。注意 **framescript 的右键菜单抑制逻辑必须同批按 pref 门控**——chrome 侧关掉只是不画线不执行动作，frame script 照样 armed 后 preventDefault contextmenu（右键菜单消失）。超级拖拽保留默认开。
32. **"授权成功却提示未登录"真凶 = `Services.tm.setTimeout` 不存在**：设备流轮询循环第一行 sleep 就抛 TypeError → 轮询静默死亡（beginLogin 的 catch 只 reportError 到 stderr）→ 用户在 GitHub 点了授权，本地令牌从未入库。修=改用窗口级 setTimeout。同批轮询加固：网络抖动 continue 重试（不再一次失败即死）、expires_in 截止、abort 监听 once。
33. **esr153 API 迁移清单（同步体系全量踩齐）**：①`logins.addLogin/removeLogin` → `addLoginAsync/removeLoginAsync`；②`PlacesUtils.bookmarks.addObserver` → `PlacesObservers.addListener(事件数组, 纯函数监听器)`（监听器收 events 数组；**无 "bookmark-changed"**，合法集=added/removed/moved/title-changed/url-changed）；③`bookmarks.fetch()` 无参全树 → `promiseBookmarksTree(rootGuid)`（节点 **uri 是 href 字符串**非 URL 对象、type 是 flavor 字符串、**typeCode 才是数值**、children 在 `.children`）；④远端 GET 必须 `cache: "no-store"`——HTTP 缓存会吐旧 bookmarks.json 导致"推了但拉不到"。同步体系：syncNow 防重入（_syncing，观察者与周期器并发）；周期同步 pref `vela.github.sync.intervalSec` 默认 900s（观察者负责变化即同步，周期器兜底）；跨窗口登录态=Services.obs 的 vela-github:login/logout 观察者刷新各窗口内存态。
34. **自动化自测（VELA_SELFTEST）**：`VELA_SELFTEST=1 VELA_TEST_TOKEN=$(gh auth token) ./dist/.../firefox -profile /tmp/xx -no-remote` 启动即跑端到端：令牌注入→refreshProfile→手动 syncNow→插书签验观察者自动同步→间隔调 5s 验周期同步→PASS/FAIL dump。独立仓 `vela-sync-selftest`（pref vela.github.sync.repo）不污染真实 vela-sync。grep "VELA_TEST" 看结果。
